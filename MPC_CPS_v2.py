...
class MPC():
  def __init__(self, mean_pos):
    ...
    self.Iyy        = 0.029125
    self.Izz        = 0.055225
    self.Ax         = 0.000175# drag force coefficients for velocities in the corresponding directions of the inertial frame.
    self.Ay         = 0.000175
    self.Az         = 0.000175
    self.k          = 8.54858e-06  # rotor_force_constant
    self.b          = 1e-06# rotor_moment_constant
    ...
    self.dz_max = 0.5; self.dz_min = -self.dz_max
    ...
    # # self.x_init = 0.0; self.y_init = 0.0;  
    # self.z_init = 2.0
    ...
    self.x_init = mean_pos.pose.position.x
    self.y_init = mean_pos.pose.position.y
    self.z_init = mean_pos.pose.position.z
    
    roll, pitch, yaw = euler_from_quaternion(mean_pos.pose.orientation.x, mean_pos.pose.orientation.y, mean_pos.pose.orientation.z, mean_pos.pose.orientation.w)
    
    self.phi_init = roll
    self.theta_init = pitch
    self.psi_init = yaw
    
    self.state_init = ca.DM([self.x_init, self.y_init, self.z_init, self.phi_init, self.theta_init, self.psi_init, self.dx_init, self.dy_init, self.dz_init, self.dphi_init, self.dtheta_init, self.dpsi_init])
  ...
  def setting_reference(self, ref):
    ...
    for k in range(self.N):
      ...
      self.args['p'][(self.n_states + self.n_controls) * k + 15] = 0.0
      self.args['p'][(self.n_states + self.n_controls) * k + 16] = 0.0
      self.args['p'][(self.n_states + self.n_controls) * k + 17] = 0.0
...
class ROS():
  def __init__(self, GPS):
    ...
    self.mean_pos_sub = rospy.Subscriber(self.robot_name + '/mean_pos', PoseStamped, self.mean_pos_callback, queue_size = 1)
    ...
    self.predict_pose_pub = rospy.Publisher(self.robot_name + '/predict_path', Path, queue_size = 1)
    
    self.state = State()
    self.ref_path = Path()
    self.mean_pos = PoseStamped()
    self.state_queue = Queue()
    self.ref_path_queue = Queue()
    self.mean_pos_queue = Queue()
  ...
  def odom_callback(self, msg):
    ...
    self.robot_ori = euler_from_quaternion(q)
  ...
  def mean_pos_callback(self, msg):
    self.mean_pos_queue.put(msg)
    self.mean_pos = msg
  ...
  def predict_pose_publish(self, X0, N_):
    predict_path = PoseArray()
    ...
    for i in range(N_):
      ...
      q_normaliazed = q / np.linalg.norm(q)
      pose = Pose()
      pose.poses.position.x = X0[:, i][0]
      pose.poses.position.y = X0[:, i][1]
      pose.poses.position.z = X0[:, i][2]
      pose.poses.orientation.x = q_normaliazed[0]
      pose.poses.orientation.y = q_normaliazed[1]
      pose.poses.orientation.z = q_normaliazed[2]
      pose.poses.orientation.w = q_normaliazed[3]
      predict_path.poses.append(pose)
      
    self.predict_pose_pub.publish(predict_path)
...
def main():
  ...
  while 1:
    if ros.mean_pos_queue.empty():
      print(ros.robot_name + 'mean pos is not subscribed..')
      continue
    else: break
  
  mpc = MPC(ros.mean_pos)
  ...
  while mpc.while_loop_condition():
    ...
    # ros.predict_pose_publish(result_X0, N_)
